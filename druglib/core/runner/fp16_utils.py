...
try:
    ...
    # and used; otherwise, auto fp16 will adopt druglib's implementation.
    # Note that when PyTorch >= 1.6.0, we still cast tensor types to fp16 manually,
    # so the behavior may be not be consistent with real amp.
...
def cast_tensor_type(
    ...
    Note:
        `cast_tersor_type` will only convert the torch.Tensor which is consistent with `src_type` to the `dst_type`.
            it avoids some potential problems. For example, `cast_tensor_type(inputs, torch.float, torch.half)`
            will convert all tensors in inputs to `torch.half` including those originally in `torch.Int` or other types,
    ...
    elif isinstance(inputs, (str, np.ndarray)):
        return inputs
...
def auto_fp16(
    ...
    backend, otherwise, original druglib implementation will be adopted.

    Args:
        apply_to:Optional[Iterable]: The argument names to be converted.
    ...
    def auto_fp16_wrapper(old_func):
        ...
        def new_func(*args, **kwargs):
            ...
            new_args = []
            if args:
                args_names = args_info.args[:len(args)]
                for i, args_name in enumerate(args_names):
                    if args_name in args_to_cast:
                        new_args.append(
            ...
            new_kwargs = {}
            if kwargs:
                for args_name, args_value in kwargs.items():
                    if args_name in args_to_cast:
                        new_kwargs[args_name] = cast_tensor_type(
                            args_value, torch.float, torch.half
                        )
                    else:
                        new_kwargs[args_name] = args_value
...
def force_fp32(
    ...
    This decorator is useful when you write custom modules and want to support
        ...
        torch.cuda.amp is used as the backend, otherwise, original druglib
    ...
    def force_fp32_wrapper(old_func) -> Callable:
...
class LossScaler:
    ...
    Args:
        init_scale:Union[int, float]: Initial loss scale value, defaults to 2**32.
    ...
    def __init__(
            self,
            init_scale:Union[int, float] = 2**32,
            mode:str = 'dynamic',
    ...
    @property
    def loss_scale(self) -> Union[int, float]:
