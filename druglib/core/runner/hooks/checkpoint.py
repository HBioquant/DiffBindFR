...
from druglib.utils import FileClient
...
class CheckpointHook(Hook):
    ...
    ::warning::
        ``out_dir`` indicates the root directory and the final path to save checkpoint
        is the concatenation of ``out_dir`` and the last level directory of
    ...
    def _save_checkpoint(self, runner):
        ...
        if self.max_keep_ckpts > 0:
            ...
            redundant_ckpt_ids = range(current_ckpt - self.max_keep_ckpts * self.interval, 0, -self.interval)
            filename_tmpl = self.args.get("filename_tmpl", name)
            for _step in redundant_ckpt_ids:
                ckpt_pth = self.file_client.join_path(
                    self.out_dir,
                    filename_tmpl.format(_step),
                )
                if self.file_client.isfile(ckpt_pth):
                    self.file_client.remove(ckpt_pth)
                else:
                    break
