...
class EMAHook(Hook):
    ...
    def __init__(
            ...
            momentum_fun: Optional[Callable] = None,
    ):
        ...
        self.ckpt = resume_from
        self.momentum_fun = momentum_fun

    def before_run(self, runner):
        """
        To resume model with its ema parameters more friendly.
    ...
    def after_train_epoch(self, runner):
        """
        Load parameter values from ema backup to model before the
        EvalHook.
        That say, EvalHook use param_ema_buffer data
        """
        self._swap_ema_parameters()

    def before_train_epoch(self, runner):
        """
        Recover model's parameter from ema backup after last epoch's
        EvalHook.
        That say, next training continues to use that last one iteration
        updated parameters instead of param_ema_buffer data
        """
        self._swap_ema_parameters()
...
@HOOKS.register_module()
class ExpMomentumEMAHook(EMAHook):
    """
...
@HOOKS.register_module()
class LinearMomentumEMAHook(EMAHook):
    """
