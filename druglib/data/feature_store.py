...
# Copied from https://github.com/pyg-team/pytorch_geometric/blob/master/torch_geometric/data/feature_store.py
import copy
...
from .typing import FeatureTensorType, IndexType
...
class TensorAttr(CastMixin):
    """
    Defines the attributes of a class:`FeatureStore` tensor; in particular, all the
        parameters necessary to uniquely identify a tensor from the feature store.
    ...
    # Note that index will not be wrapped by :obj:`DataContainer`
...
class AttrView(CastMixin):
    ...
    def __getattr__(self, key: Any) -> Union['AttrView', FeatureTensorType]:
        """
        Sets the first unset field of the backing :class:`TensorAttr`
    ...
    def __setattr__(self, key: str, value: Any):
        """
        Supports attribute assignment to the backing :class:`TensorAttr` of
    ...
    def __call__(self) -> FeatureTensorType:
        ...
        .. code-block:: python
            ...
            This will ignore whether the index is set
        """
    ...
    def __eq__(self, obj: Any) -> bool:
        """
        Compares two :class:`AttrView` objects by checking equality of
...
class FeatureStore:
    def __init__(self, tensor_attr_cls: Any = TensorAttr):
        """
        Initializes the feature store. Implementor classes can customize
    ...
    def put_tensor(self, tensor: FeatureTensorType, *args, **kwargs) -> bool:
        """
        Synchronously adds a :class:`FeatureTensorType` object to the feature store.
        Args:
    ...
    @abstractmethod
    def _get_tensor(self, attr: TensorAttr) -> Optional[DataContainer]:
        """To be implemented by :class:`FeatureStore` subclasses."""
        pass

    def get_tensor(self, *args, **kwargs) -> DataContainer:
        """
        Synchronously obtains a :class:`DataContainer` object from the
            feature store. Feature store implementors guarantee that the call
            :obj:`get_tensor(put_tensor(tensor, attr), attr).data = tensor` holds.
        Args:
            **attr (TensorAttr): Any relevant tensor attributes that correspond
                to the feature tensor. See the :class:`TensorAttr`
                documentation for required and optional attributes. It is the
                ...
                from a :class:`TensorAttr` object.
        Returns:
            DataContainer: a DataContainer of indexed Tensor
        Raises:
        ...
        tensor: DataContainer = self._get_tensor(attr)
        if tensor is None:
    ...
    def remove_tensor(self, *args, **kwargs) -> bool:
        """
        Removes a :obj:`FeatureTensorType` object from the feature store.
        Args:
            **attr (TensorAttr): Any relevant tensor attributes that correspond
                to the feature tensor. See the :class:`TensorAttr`
                documentation for required and optional attributes. It is the
    ...
    def update_tensor(
        ...
        """
        Updates a :class:`FeatureTensorType` object with a new value.
    ...
    def view(self, *args, **kwargs) -> AttrView:
        """
        Returns an :class:`AttrView` of the feature store, with the defined
            attributes set.
        """
        attr = self._tensor_attr_cls.cast(*args, **kwargs)
    ...
    def __getitem__(self, key: TensorAttr) -> Any:
        """
        Supports pythonic indexing into the feature store. In particular,
