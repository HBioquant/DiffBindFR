...
# Copied from https://github.com/pyg-team/pytorch_geometric/blob/master/torch_geometric/data/graph_store.py
import copy
import warnings
...
from .typing import Adj, EdgeTensorType, OptTensor
...
class GraphStore:
    def __init__(self, edge_attr_cls: Any = EdgeAttr):
        """
        Initializes the graph store. Implementor classes can customize the
            ordering and required nature of their :class:`EdgeAttr` edge
            attributes by subclassing :class:`EdgeAttr` and passing the
            subclass as :obj:`edge_attr_cls`.
        """
        super().__init__()
    ...
    def put_edge_index(
        ...
        Args:
            tensor(EdgeTensorType): an edge_index in a format specified in attr.
        ...
        # from druglib import is_seq_of
        # # judge exact input type and transform the input edge_index to DataContainer
        # if is_seq_of(edge_attr, Tensor):
        #     row, col = DataContainer(data = edge_attr[0], is_graph = True, stack = False), \
        #                DataContainer(data = edge_attr[1], is_graph = True, stack = False)
        #     edge_attr = (row, col)
        # elif is_seq_of(edge_attr, DataContainer):
        #     pass
        # else:
        #     # find `which one` is DataContainer
        #     which_one = 0 if isinstance(edge_attr[0], DataContainer) else 1
        #     edge_attr[1 - which_one] = edge_attr[which_one].update(edge_attr[1 - which_one])
    ...
    def coo(
        ...
        """
        Converts the edge indices in the graph store to COO format,
    ...
    def csr(
        ...
        """
        Converts the edge indices in the graph store to CSR format,
            optionally storing the converted edge indices in the graph store.
        """
    ...
    def csc(
        ...
        """
        Converts the edge indices in the graph store to CSC format,
            optionally storing the converted edge indices in the graph store.
        """
...
def edge_tensor_type_to_adj_type(
    ...
    """Converts an EdgeTensorType tensor tuple to a PyG Adj tensor."""
...
def adj_type_to_edge_tensor_type(layout: EdgeLayout,
    ...
    """Converts a PyG Adj tensor to an EdgeTensorType equivalent."""
